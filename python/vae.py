...
class VAE(object):
    def __init__(self, x, dataset, n_input, n_z, network_architecture,
        ...
        with tf.name_scope('input'):
            ...
            self.x_binarized = tf.tile(self.x_binarized,
                                       [1, self.n_samples, 1])
        with tf.name_scope('prior_probs'):
            shape = [self.batch_size, self.n_samples, self.n_ary * self.n_z]
            self.prior_probs = (1. / self.n_ary) * tf.ones(shape)

        self.__create_network()

        self.__create_loss_optimizer()

        self.init_weights = tf.variables_initializer(
            self.flat_weights.values())
    ...
    def _initialize_weights(self):
        ...
        for nn_part, layers in self.network_architecture.items():
            for name, (n_in, n_out) in layers.items():
                weights[nn_part][name] = init_weights(  # noqa
                    n_in, n_out, name=nn_part + '_' + name)
        self.encoder_weights = list(chain.from_iterable(
            weights['encoder'].values()))
        self.decoder_weights = list(chain.from_iterable(
            weights['decoder'].values()))
        self.flat_weights = flatten(weights)  # noqa
        return weights
    ...
    def _create_encoder_part(self):
        ...
        x = self.x_binarized
        x, sh = unroll_tensor(x)  # noqa

        with tf.name_scope('encoder'):
            self.encoder_layer1 = build_layer(  # noqa
                x, *weights['encoder']['h1'],
                nonlinearity=self.nonlinearity, name='encoder_h1')
            self.encoder_layer2 = build_layer(  # noqa
                self.encoder_layer1, *weights['encoder']['h2'],
                nonlinearity=self.nonlinearity, name='encoder_h2')
        with tf.name_scope('stochastic_layer'):
            self.z_mean = build_layer(  # noqa
                self.encoder_layer2, *weights['encoder']['out_mean'],
                name='encoder_out_mean')
            self.z_mean = roll_tensor(self.z_mean, sh)  # noqa

            if self.encoder_distribution == 'gaussian':
                self.z_log_sigma_sq = build_layer(  # noqa
                    self.encoder_layer2,
                    *weights['encoder']['out_log_sigma_sq'],
                    name='encoder_out_log_sigma_sq')

                self.z_log_sigma_sq = roll_tensor(self.z_log_sigma_sq, sh)  # noqa
                with tf.name_scope('stochastic_node'):
                    epsilon = tf.random_normal(tf.shape(self.z_mean))
                    self.z = tf.exp(0.5 * self.z_log_sigma_sq) * epsilon
                    self.z = self.z_mean + self.z
                ...
                with tf.name_scope('stochastic_node'):
                    ...
                    self.z = tf.reshape(self.z, shape)
                with tf.name_scope('z_probs'):
                    self.z_probs = tf.reshape(
                        tf.nn.softmax(self.z_mean), shape)
                with tf.name_scope('reshape'):
    ...
    def _create_decoder_part(self, z):
        ...
        with tf.name_scope('decoder'):
            z, sh = unroll_tensor(z)  # noqa
            decoder_layer1 = build_layer(  # noqa
                z, *weights['decoder']['h1'],
                nonlinearity=self.nonlinearity, name='decoder_h1')
            decoder_layer2 = build_layer(  # noqa
                decoder_layer1, *weights['decoder']['h2'],
                nonlinearity=self.nonlinearity, name='decoder_h2')
            x_reconst = build_layer(  # noqa
                decoder_layer2, *weights['decoder']['out_mean'],
                name='decoder_out_mean')
            x_reconst = roll_tensor(x_reconst, sh)  # noqa
        return x_reconst
    ...
    def _create_loss(self):
        with tf.name_scope('loss'):
            if self.decoder_distribution == 'multinomial':
                self.decoder_log_density = compute_log_density(  # noqa
                    x=self.x_binarized, logits=self.x_reconst,
                    distribution=self.decoder_distribution,
                    name='decoder_log_density')
            elif self.decoder_distribution == 'gaussian':
                self.decoder_log_density = compute_log_density(  # noqa
                    x=self.x_binarized, mu=tf.nn.softmax(self.x_reconst),
            ...
            if self.encoder_distribution == 'multinomial':
                with tf.name_scope('reshape'):
                    shape = [self.batch_size, self.n_samples,
                             self.n_z, self.n_ary]
                    z = tf.reshape(self.z, shape)
                    z_shape = tf.shape(z)
                    z_mean, z_probs, prior_probs = tuple(map(
                        lambda x: tf.reshape(x, z_shape),
                        [self.z_mean, self.z_probs, self.prior_probs]))
                self.encoder_log_density = compute_log_density(  # noqa
                    x=z, logits=z_mean, probs=z_probs, prior_probs=prior_probs,
                    distribution=self.encoder_distribution,
                    name='encoder_log_density')
                self.kl_divergency = compute_kl_divergency(  # noqa
                    x=z, logits=z_mean, probs=z_probs, prior_probs=prior_probs,
                    distribution=self.encoder_distribution,
                    name='kl_divergency')
            elif self.encoder_distribution == 'gaussian':
                self.encoder_log_density = compute_log_density(  # noqa
                    x=self.z, mu=self.z_mean,
                ...
                self.kl_divergency = compute_kl_divergency(  # noqa
                    ...
                    distribution=self.encoder_distribution,
                    name='kl_divergency')
            self.multisample_elbo = - compute_multisample_elbo(  # noqa
                self.decoder_log_density, self.kl_divergency)

            self.cost_for_decoder_weights = tf.reduce_mean(
    ...
    def _create_optimizer(self):
        with tf.name_scope('optimizer'):
            with tf.name_scope('decoder_optimizer'):
                ...
                self.decoder_minimizer = self.decoder_optimizer.minimize(
                    self.cost_for_decoder_weights,
                    var_list=self.decoder_weights)
            with tf.name_scope('encoder_optimizer'):
                self.encoder_optimizer = tf.train.AdamOptimizer(
                    learning_rate=self.learning_rate, epsilon=1e-4)
                self.encoder_minimizer = self.encoder_optimizer.minimize(
                    self.cost_for_encoder_weights,
                    var_list=self.encoder_weights)
            with tf.name_scope('gradients'):
                self.optimizer = tf.train.AdamOptimizer(
                    learning_rate=self.learning_rate, epsilon=1e-4)
                self.decoder_gradients = self.optimizer.compute_gradients(
                    self.cost_for_decoder_weights,
                    var_list=self.decoder_weights)
                self.encoder_gradients = self.optimizer.compute_gradients(
                    self.cost_for_encoder_weights,
                    var_list=self.encoder_weights)
    ...
    def parameters(self, lr=None, without_lr=False, **kwargs):
        learning_rate = lr or self.learning_rate_value
    ...
    def get_decoder_gradients(self, X, learning_rate=None, n_samples=None):
        ...
        gradients = self.sess.run(self.decoder_gradients,
                                  feed_dict={
                                      self.x: X,
                                      self.learning_rate: learning_rate,
                                      self.n_samples: n_samples,
    ...
    def get_decoder_log_density(self, X):
        return self.sess.run(tf.reduce_mean(self.decoder_log_density),
                             feed_dict={self.x: X})
...
class VIMCOVAE(VAE):
    ...
    def _create_loss_optimizer(self):
        ...
        self.multisample_elbo = - compute_multisample_elbo(  # noqa
            self.decoder_log_density, self.kl_divergency)
        self.multisample_elbo_vimco = -compute_multisample_elbo(  # noqa
            self.decoder_log_density, self.kl_divergency, is_vimco=True)
        self.multisample_elbo_vimco = tf.stop_gradient(
            tf.transpose(self.multisample_elbo_vimco))
        self.vimco_cost = tf.reduce_sum(self.encoder_log_density
                                        * self.multisample_elbo_vimco, 1)
...
class NVILVAE(VAE):
    ...
    def _create_loss_optimizer(self):
        ...
        if self.n_samples_value == 0:
            ...
            self.nvil_cost -= self.deterministic_term
            self.cost_for_encoder_weights = tf.reduce_mean(
            ...
            self.cost_for_baseline = tf.reduce_mean(
                (self.multisample_elbo - self.baseline)**2)
        self._create_optimizer()

        self.baseline_optimizer = tf.train.AdamOptimizer(
            learning_rate=self.baseline_learning_rate)
        self.baseline_minimizer = self.decoder_optimizer.minimize(
            self.cost_for_baseline, var_list=self.baseline_weights)

    def partial_fit(self, learning_rate_decay=1.0,
                    n_samples=None, is_train=True):
        dict_of_tensors, feed_dict = super().partial_fit(
            learning_rate_decay, n_samples, is_train)
    ...
    def _create_loss_optimizer(self):
        ...
        if self.n_samples_value == 0:
            ...
            self.multisample_elbo_adjusted -= tf.reduce_sum(
                self.linear_part, -1)
            self.multisample_elbo_adjusted = tf.stop_gradient(
                self.multisample_elbo_adjusted)
...
class GumbelSoftmaxTrickVAE(VAE):
    def __init__(self, *args, **kwargs):
        ...
        if self.encoder_distribution != 'multinomial':
            ...
            error_message = error_message.format(self.encoder_distribution)
    ...
    def _create_network(self):
        ...
        shape = [self.batch_size, self.n_samples,
                 self.n_ary * self.n_z]
        self.z_probs = tf.reshape(tf.nn.softmax(self.z_mean), shape)

        def true_fn():
            return gumbel_softmax(self.z_mean, self.temperature)  # noqa

        def false_fn():
            return tf.one_hot(tf.squeeze(
                tf.multinomial(self.z_mean, 1), 1), self.n_ary, 1.0, 0.0)

        self.z = tf.cond(tf.equal(self.is_train, tf.constant(1)),
                         true_fn, false_fn)
    ...
    def parameters(self, *args, tmp=None, **kwargs):
        params = super().parameters(*args, **kwargs)
        temperature = tmp or self.temperature
